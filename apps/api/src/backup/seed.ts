/**
 * The demo dataset: a realistic, multi-year job search.
 *
 * Not just filler. The data is arranged so the two features that are hard to demonstrate on
 * an empty database actually have something to find:
 *
 * - **Duplicate detection**: several companies appear more than once, including one with two
 *   spellings ("Spotify" and "Spotify AB") and one where the same role was applied for twice
 *   a year apart.
...
/**
 * Writes the demo dataset into `repos`. Does not check whether the database is already
 * empty — callers (the CLI's `--force` guard, the Settings page's "empty only" gate) own
 * that decision, since a script and a running server want different defaults there.
 */
export async function seedDemoData(repos: Repos): Promise<SeedResult> {
