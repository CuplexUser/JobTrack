...
import { parseDateOnly, todayDateOnly, type JobApplicationView, type JobOpeningView } from '@jobtrack/shared';
...
export async function getAgenda(repos: Repos): Promise<AgendaPayload> {
  ...
  const [attention, openings] = await Promise.all([needsAttention(repos), listOpenings(repos)]);
  ...
  return { today, followUps: attention.followUps, goneQuiet: attention.stale, idleOpenings };
}
